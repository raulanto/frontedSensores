<template>
    <div>
        ...
        <template v-else-if="valoresArray.length > 0">
            <CircularGauge :series="porcentaje" :labels="Datos"
            ></CircularGauge>
            <button type="button"
...
interface ChartData {
    valor: number; // Aseguramos que el tipo es number
}

const valoresArray = ref<number[]>([]);
const valorFecha = ref<string[]>([]);
const porcentaje = ref<any>(0);
...
async function fetchData() {
    ...
    try {
        ...
        valoresArray.value = lecturaStore.datosLectura.map(item => Number(item.valor));
        valorFecha.value = lecturaStore.datosLectura.map(item => item.created_at);

        // Llamar a la función para calcular el porcentaje después de cargar los datos
        calcularPorcentaje();
    } catch (error) {
...
function calcularPorcentaje() {
    ...
    let sumatoria = array.reduce((acc, valor) => acc + valor, 0);
    let promedio = sumatoria / array.length;

    let elementosCumplenCondicion = array.filter(valor => valor > promedio).length;

    let resultado =(elementosCumplenCondicion / array.length) * 100;
    porcentaje.value = resultado.toFixed(1)

    console.log(`Porcentaje de valores mayores que el promedio (${promedio.toFixed(2)}): ${porcentaje.value.toFixed(2)}%`);
}
</script>
